...
<body>
    ...
    <div class="container">
        ...
        <!-- Add Heat Index card here -->
        <div class="temperature-card" id="heat-index-card">
            <h2>Heat Index</h2>
            <div class="value" id="heatIndex">--°C</div>
        </div>
    ...
    <script>
        ...
        function onMessage(event) {
            try {
                ...
                document.getElementById('heatIndex').innerHTML = data.heatIndex.toFixed(1) + '&deg;C';
                ...
                // Update temperature colors for both temperature and heat index cards
                const [color1, color2] = getTemperatureColors(data.temperature);
                const tempCard = document.getElementById('temp-card');
                const weatherCard = document.querySelector('.weather-card');
                const heatIndexCard = document.getElementById('heat-index-card');
                
                [tempCard, weatherCard, heatIndexCard].forEach(card => {
                    card.style.setProperty('--temp-color-1', color1);
                    card.style.setProperty('--temp-color-2', color2);
                });
